...
//(Mat& m_neigh_Data, Vec& v_vol, std::vector<std::vector<PetscInt>>& vv_parts, Mat& m_parts, Mat& m_centers){
    ...
    std::vector<std::vector<PetscInt>> vv_parts(3);
    read_parts(vv_parts, m_parts,1);

//    MainRecursion mr("fake_test_file_name")  ;
    MainRecursion mr;
    mr.calc_center(m_dt,v_vol,vv_parts,m_parts,m_centers);
...
void ut_MR::load_matrix(const char * f_name, Mat& m_data, bool print){
...
void ut_MR::load_vec(const char * f_name, Vec& v_input, bool print){

...
//                    std::cout << "[MC] test data file is removed successfully from " << test_file << std::endl;
    ...
    case 1:
    {
        std::cout << "[MC] Regression has not added yet!" << std::endl;
        break;

    }

    case 2: //Community Detection
    {
        ...
        std::cout << "[MLCDM] prefix: " << prefix << std::endl;
        std::string full_data {prefix + "_zsc_data.dat"};
        std::cout << "[MLCDM] full_data:<<"<< full_data << std::endl;

        Loader ld;
        m_full_data = ld.read_input_matrix(full_data);

        // - - - - - load full indices matrics - - - - -
        Mat m_full_NN_indices,m_full_NN_dists;
        std::string full_indices {prefix + "_zsc_data_indices.dat"};
        std::string full_dists {prefix + "_zsc_data_dists.dat"};
        m_full_NN_indices = ld.read_input_matrix(full_indices);
        m_full_NN_dists = ld.read_input_matrix(full_dists);
        // - - - - - load WA matrix - - - - -
        Mat m_WA;
        ld.create_WA_matrix(m_full_NN_indices,m_full_NN_dists,m_WA,"full");
        PetscInt num_row;
        MatGetSize(m_WA, &num_row, NULL);
        Vec v_vol;
        v_vol = ld.init_volume(1,num_row);
        //====================== Multilevel Clustring ===============================
        ETimer t_solver;

        MainRecursion multilevel_solver;        // the test data file name is initialized in the constructor of MR class
        int k = 2;  // #add num_clusters to parameters
        multilevel_solver.main_community_detection(m_full_data, m_WA, v_vol, k, 0);
//        Refinement rf;
//        rf.add_best_model(v_ref_results);
        //free the matrices
        MatDestroy(&m_WA);
        ...
        //destory the remaining objects to free the memory
        MatDestroy(&m_full_data);
        MatDestroy(&m_full_NN_indices);
        MatDestroy(&m_full_NN_dists);

        printf("[Main] ML Clustering finished successfully!\n");
//        exit(1);

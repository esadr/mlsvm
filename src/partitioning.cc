 ...
 * output:
 *          vv_parts
 *          m_parts: row i represent part i and the nnz in row i
 *                      are the points which are belong to part i
 */

int Partitioning::get_parts(Mat& m_WA, Vec& v_vol, int num_req_parts, int level,
                            std::vector<std::vector<PetscInt>>& vv_parts,
                            Mat& m_parts, unsigned int metis_seed){
    std::string cfid = "[PR][GPA]";
    // - - - - - - define metis parameters - - - - - -
    PetscInt num_vertex, num_edge=0, num_col;
    MatGetSize(m_WA, &num_vertex, &num_col);

    //metis API needs to count both (i,j) and (j,i) and later it double it
    //  again which is 4 times unique edges in my case
    PetscInt            i,j, ncols;
    const PetscInt      *cols;
    ...
    cout << cfid << " number of parts are: " << nparts << endl;
#endif
...
//    vwgt= NULL;
    ...
    // Number of edges are less than below because I remove the loop to itself,
    //  but the below value is useful for malloc
    printf("[PR][get_parts] num_vertex:%d, (nnz in WA which is double of unique num edges) num_edge:%d\n",num_vertex, num_edge);
    #if dbl_PR_get_parts >=7    //default is 7
        cout  << "[PR][get_parts] m_neigh_WA matrix:\n";
...
//    scale_edge_weights(adjwgt, num_edge, arr_adjwgt_);
    ...
    int result;
    try{
        // This one is not working, for the second class
...
//                                       ubvec, options, &objval, part);
        ...
        cout << cfid << " Metis faced a problem " << endl;
    }

#if dbl_PR_get_parts >= 3    //default is 3
    cout << cfid << " Metis result: " << result << endl;
    #if dbl_PR_get_parts >= 7    //default is 7
    cout << cfid << " [dev-test] [part index, selected partition]" << endl;
    for(i=0; i<num_vertex ;i++){
        printf("[%d:%d], ",i, part[i]);
    }
    printf("\n\n");
    ...
    if(curr_adj_ind_ > num_edge) {
        cout << cfid << "curr_adj_ind_:" << curr_adj_ind_ <<" is larger "<<
                "than number of non zeros in meigh_WA:" << num_edge <<
                " which is wrong" << endl;
        exit(1);
    }
    if(result != 1){    // 1 means ok (I email for confirmation)
        cout << cfid << "Metis failed, return value is:"<< result <<"!" <<endl;
        exit(1);
    }

    //prepare the output
    MatCreateSeqAIJ(PETSC_COMM_SELF, num_req_parts, num_vertex , num_vertex,
                    PETSC_NULL, &m_parts);
    // part[i] value determine the partition, and i is the index of
    //  the point in data matrix
    for(i=0; i<num_vertex ;i++){
    ...
    for(i=0; i< num_req_parts; i++){
        if(vv_parts[i].size() != 0){
            ...
            cout << cfid << "empty partition at index:" << i <<
                    ", Exit!" << endl;
            CommonFuncs cf;
            cf.exp_matrix(m_WA, "./data", "m_neigh_WA.dat","get_part");
            cf.exp_vector(v_vol, "./data", "v_neigh_vol.dat","get_part");
            cout << "number of partitions was " << num_req_parts <<
                    ", m_WA, v_vol are exported!" << endl;
            exit(1);
        }
    }
    ...
    for(i=0; i < num_req_parts; i++){
        ...
        printf("%s Result Parts of group:%d, size:%d\n",cfid,i, curr_idx_size);
        for(int j = 0; j < curr_idx_size ; j++){
            printf("%d,",vv_parts[i][j]) ;
        }
        cout << endl;
    }
#endif
...
/*
 * - If weights are inversed in the beginning, the higher values
 *      (inversed weights) means stronger relation and are closer to each other
 * - The metis is going to minimize the edge cut, so it keeps the heavy edge
 *      weights inside the partitions and there is no need to inverse the
 *      weights again, but if the values were distances, we need to inverse them
 */
void Partitioning::scale_edge_weights(idx_t *adjwgt, PetscInt num_edge,
                                      PetscScalar *arr_adjwgt){
    bool weight_inversed = paramsInst->get_inverse_weight() ;
    PetscInt i;
    ...
    if (weight_inversed ){
        ...
        tmp_sum += num_edge * 4;
        // max number for 64 bit is:  9,223,372,036,854,775,807,
        //  but I used the max of 32 bits
        scale_rate = (4294967295 / tmp_sum) * 0.2;
#if dbl_PR_SEW >=3    //default is 3
        printf("[PR][SEW] total sum before scaling:%g, scale_rate:%g \n",
               tmp_sum, scale_rate);
#endif
        for(i=0; i < num_edge; i++){
            if(arr_adjwgt[i] > -4){
                // range is larger than 1 and less than 800
                adjwgt[i] = round( (arr_adjwgt[i] + 4) * scale_rate);
            }else{         // handle large negative values
                           //   (outliers in negative side)
                adjwgt[i] = 0;
            }
#if dbl_PR_SEW >=9    //default is 9
            printf("[PR][SEW] not inversing i:%d, adjwgt[i]:%d \n",
                   i, adjwgt[i]);
#endif
        }
    }else{
...
//            adjwgt[i] = round ( (1.0 / (arr_adjwgt[i] + 0.0001)) * 10000 )  ;
            adjwgt[i] = 1;
#if dbl_PR_SEW >=9    //default is 9
            printf("[PR][SEW] inversing i:%d, adjwgt[i]:%d \n",
                        i, adjwgt[i]);
#endif
        }
    }
...
/*
 * output:
 *          m_parst: a matrix with 1 row with number of points as
 *              features, all of them are 1
 */
 ...
 */
void Partitioning::calc_single_center(Mat& m_neigh_Data, Vec& v_vol,
                                  Mat& m_center,
...
//    cout << "[PR][CSC] sum vol:" << sum_vol << endl;
    ...
    if(sum_normalized_vol == 0) {
        cout << "[PR][CSC] sum normalized volume is zero which cause"<<
                " division by zero!, exit " << endl;
        exit(1);
    }
    ...
    Mat m_VnormDiag;
    MatCreateSeqAIJ(PETSC_COMM_SELF,num_point,num_point,1,PETSC_NULL,
                    &m_VnormDiag);
    ...
    //---- Data_Vol = VD * Data #4 -----
    Mat m_Data_Vol;
    MatMatMult(m_VnormDiag, m_neigh_Data, MAT_INITIAL_MATRIX,
               PETSC_DEFAULT,&m_Data_Vol);
    MatDestroy(&m_VnormDiag);

    //---- Transpose Data_Vol #5 -----
    // I continue to comment on columns which is the real logic,
    //  but in code I use row because of CSR matrix performance
    ...
    MatCreateSeqAIJ(PETSC_COMM_SELF, 1, num_features , num_features,PETSC_NULL,
                    &m_center);
    for(int i=0; i<num_features; i++){
//        cout << "[PR][CSC] arr_sum_DV_col[i]: " << arr_sum_DV_col[i] << ", calc: " <<
//                     arr_sum_DV_col[i]/sum_normalized_vol << " at i: " << i << endl;
        MatSetValue(m_center, 0, i, arr_sum_DV_col[i]/sum_normalized_vol,
                    INSERT_VALUES );
    }
    ...
    MatView(m_center,PETSC_VIEWER_STDOUT_WORLD);
#endif
}
 ...
 * I consider the volumes in the calculating the centers and I normalize
 *   the center using sum of all volumes in this part
 * @input: neigh_Data, neigh_vol, and the partitions
 * @output: matrix with parts number of rows and for columns the same number
 *      of features as WA
 */
void Partitioning::calc_center(Mat& m_neigh_Data, Vec& v_neigh_vol,
                               std::vector<std::vector<PetscInt>>& vv_parts,
    ...
    //---- Data_Vol = VD * Data #2 -----
    Mat m_Data_Vol;
    MatMatMult(m_VnormDiag, m_neigh_Data, MAT_INITIAL_MATRIX,PETSC_DEFAULT,
               &m_Data_Vol);
    MatDestroy(&m_VnormDiag);
    ...
    cout  << "[PR][calc_center] m_Sum_Col_Part matrix:\n";
...
//    std::vector<PetscScalar> v_sum_norm_vol(num_parts);
    ...
    for(i=0; i <num_parts; i++){
        ...
        for(j=0; j < vv_parts[i].size(); j++){
            // vv_parts[i][j] is the index of j-th point in partition i,
            //  and we get the volume from array of v_neigh_vol_norm
    ...
    const PetscScalar   *vals;
    //prepare the output
    MatCreateSeqAIJ(PETSC_COMM_SELF, num_parts, num_features , num_features,
                    PETSC_NULL, &m_centers);
    ...
    for(i=0; i < num_parts; i++) {
        ...
        for(j=0; j < ncols; j++){
            ...
            //skip the partitions with zero volume (empty partitions)
            if(v_sum_vol_parts[i] != 0){
                MatSetValue(m_centers, i, cols[j], vals[j] /v_sum_vol_parts[i],
                            INSERT_VALUES);
            }
        }
...
#if dbl_PR_calc_center >=7    //default is 7
    cout  << "[PR][calc_center] m_centers matrix:\n";
...
/*
 * Calculates the Euclidean distance between all minority class(P) and
 *      majority class(N) and themselves
 * @input: centers for both classes
 * @output: m_dist is a square matrix which first #p rows and #p columns
 *      and then #n rows and #n columns
 *       - P - - - - N -
 *     P| P        P->N |
 *     N| N-> P     N   |
 *       ----
...
void Partitioning::calc_distances(int num_part_p, int num_part_n,
    ...
    MatCreateSeqDense(PETSC_COMM_SELF,num_part_p+num_part_n,
                      num_part_p+num_part_n,NULL, &m_dist);
    ...
    cout  << "[PR][calc_distances] m_centers_p matrix:\n";
    MatView(m_centers_p,PETSC_VIEWER_STDOUT_WORLD);
    cout  << "[PR][calc_distances] m_centers_n matrix:\n";
    ...
    for(i=0; i< num_part_p; i++ ){
        ...
        for(j=0; j< num_part_n; j++){
            ...
            MatSetValue(m_dist,i, num_part_p + j,
                        cf.calc_euclidean_dist(ncols_p,ncols_n,cols_p,cols_n,
                                               vals_p,vals_n), INSERT_VALUES);
...
//            cout  << "[PR][calc_distances] P only i:" << i << " j:" << j << "\n";
            MatSetValue(m_dist,i, j,
                        cf.calc_euclidean_dist(ncols_p,ncols_n,cols_p,cols_n,
                                               vals_p,vals_n), INSERT_VALUES);
    ...
    for(i=0; i< num_part_n; i++ ){
        ...
        for(j=i+1; j< num_part_n; j++){
            MatGetRow(m_centers_n,j,&ncols_n, &cols_n, &vals_n);    //N row
//            cout  << "[PR][calc_distances] N only i:" << num_part_p + i <<
//                  " j:" << num_part_p + j << "\n";
            MatSetValue(m_dist,num_part_p + i,num_part_p+ j,
                        cf.calc_euclidean_dist(ncols_p,ncols_n,cols_p,cols_n,
                                               vals_p,vals_n), INSERT_VALUES);
            MatRestoreRow(m_centers_n,i,&ncols_n, &cols_n, &vals_n);
        }
...
#if dbl_PR_calc_distances >=7    //default is 7
    cout  << "[PR][calc_distances] m_dist matrix:\n";
...
//            cout<< "[PR][FG] v_groups size:" << v_groups.size() << endl;
        ...
        for(auto it=v_groups.begin(); it != v_groups.end(); it++){
            cout<< "[PR][FG] group P part index: "<< it->first <<
                   ", N part index: " << it->second << endl;
        }
#endif
        cout<< "[PR][FG] end of only 1 minotiry part!\n";
    ...
    if(num_part_n == 1){
        if(num_part_p == 1){
            ...
            // add all the P class partitions with the only one N
            //  class partition(N goes in second)
            for(int i=0; i < num_part_p; i++){
...
#endif
        cout<< "[PR][FG] end of only 1 majotiry part!\n";
        return; //stop the rest
    }

    // - - - - normal scenario with multiple partition in both class - - - -
    PetscInt i,j, ncols;
    const PetscInt    *cols;
    const PetscScalar *vals;
    ...
    for(i=0; i< num_part_p; i++ ){
        ...
        for(j=0; j< ncols; j++){
            //ignore the loop to itself, and distance to P class
            if( (cols[j] != i) && (cols[j] >= num_part_p)  ){
                v_opposite_class.push_back(
                        std::pair<int, double>(cols[j]-num_part_p, vals[j]));
            }
        }
        // - - - - sort partitions on distance - - - - -
        sort(v_opposite_class.begin(), v_opposite_class.end(),
             [](std::pair<int, double> x, std::pair<int, double> y)
             {return x.second < y.second;});
        v_groups.push_back(std::pair<int,int>(i,v_opposite_class[0].first) );
#if dbl_PR_find_groups >= 9     // default 9
        cout<< "[PR][FG] P class (normal scenario) (P:"<< i <<", N:"<<
               v_opposite_class[0].first <<")!\n";
#endif
...
//        cout<< "[PR][FG]  i:"<< i << endl;
        ...
        // - - - - create vector for opposite parts - - - - -
        for(j=0; j< ncols; j++){
              //ignore the loop to itself
            if( (cols[j] != i) && (cols[j] < num_part_p)){
                    //left bottom part of m_dist matrix(N->P)
                    v_opposite_class.push_back(
                                std::pair<int, double>(cols[j], vals[j]));
//                    cout<< "[PR][FG] inside N class part, P index:" <<
//                           cols[j] << ", dist:"<< vals[j] << endl;
                }
        }
        // - - - - sort partitions on distance - - - - -
        sort(v_opposite_class.begin(), v_opposite_class.end(),
        ...
        // in the pairs it is assumed that the first is the P_index and
        //   second is N_index for partitions
        std::pair<int, int> check_duplicate = std::pair<int, int> (
                    v_opposite_class[0].first, (i-num_part_p) ) ;
//        cout<< "[PR][FG] N class (normal scenario) check duplicate (P:"<<
//               check_duplicate.first<<", N:"<<check_duplicate.second<<")!\n";

        // ignore duplicate groups
        if(std::find(v_groups.begin(), v_groups.end(),
                     check_duplicate ) == v_groups.end() ) {
            v_groups.push_back(std::pair<int,int>(
                            v_opposite_class[0].first, (i-num_part_p)));
        }
        MatRestoreRow(m_dist,i,&ncols, &cols, &vals);
    }
    ...
    cout<< "[PR][FG] final group v_groups.size():"<< v_groups.size() << endl;
    for(auto it=v_groups.begin(); it != v_groups.end(); it++){
        cout<< "[PR][FG] final group P part index: "<< it->first <<
               ", N part index: " << it->second << endl;
    }
    #endif
    cout<< "[PR][FG] End of final group"<< endl;
#endif
...
void Partitioning::calc_avg_center(Mat& m_centers_p, Mat& m_centers_n,
                       std::vector<std::pair<int, int> >& v_groups,
    ...
    // - - - - calculate the avg centers with respect to normalizing
    //                         back using sum of volumes in partitions - - - -
    ...
    MatCreateSeqAIJ(PETSC_COMM_SELF,v_groups.size(), num_features, num_features,
                    PETSC_NULL, &m_avg_centers);

    for(it_grp=0; it_grp < v_groups.size(); it_grp++) {
        MatGetRow(m_center_vol_p, v_groups[it_grp].first, &ncols_p, &cols_p,
                  &vals_p);
        MatGetRow(m_center_vol_n, v_groups[it_grp].second, &ncols_n, &cols_n,
                  &vals_n);
        it_p_id = 0;
        it_n_id = 0;
        sum_group_volumes = v_sum_vol_parts_p[v_groups[it_grp].first] +
                v_sum_vol_parts_n[v_groups[it_grp].second];
#if dbl_PR_CAC >=9
        ...
        while( it_p_id < ncols_p && it_n_id < ncols_n){
            if(cols_p[it_p_id] == cols_n[it_n_id] ){
                MatSetValue(m_avg_centers, it_grp, cols_p[it_p_id],
                            (vals_p[it_p_id] + vals_n[it_n_id]) /
                            sum_group_volumes, INSERT_VALUES);
                it_p_id++;
                it_n_id++;
            }else{
                // P has a nnz and N doesn't have it
                if(cols_p[it_p_id] < cols_n[it_n_id] ){
                    MatSetValue(m_avg_centers, it_grp, cols_p[it_p_id],
                                (vals_p[it_p_id]) / sum_group_volumes ,
                                INSERT_VALUES);
                    it_p_id++;
                }else{   // N has a nnz and P doesn't have it
                    MatSetValue(m_avg_centers, it_grp, cols_n[it_n_id],
                                (vals_n[it_n_id]) / sum_group_volumes ,
                                INSERT_VALUES);
                    it_n_id++;
                }
            }
        }
        MatRestoreRow(m_center_vol_p, v_groups[it_grp].first, &ncols_p, &cols_p,
                      &vals_p);
        MatRestoreRow(m_center_vol_n, v_groups[it_grp].second, &ncols_n, &cols_n,
                      &vals_n);
    ...
    MatView(m_avg_centers,PETSC_VIEWER_STDOUT_WORLD);
#endif
}



void Partitioning::calc_center_volume(Mat& m_centers,
                                      std::vector<PetscScalar>& v_sum_vol_parts,
                                      Mat& m_center_vol){
    ...
    if( num_point != v_sum_vol_parts.size())    {
        cout << "[PR][CCV] mismatch size m_centers:"<< num_point <<
                ", v_vol_parts.size():"<< v_sum_vol_parts.size() << endl;
//        exit(1);
    }
...
//        cout << "\n\n [DEBUG] v_sum_vol_parts_p[i]:" <<
//                v_sum_vol_parts[i] << endl;
    ...
    MatMatMult(m_V_Diag, m_centers, MAT_INITIAL_MATRIX,PETSC_DEFAULT,
               &m_center_vol);
    MatDestroy(&m_V_Diag);
...
//        VecView(v_sum_vol_,PETSC_VIEWER_STDOUT_WORLD);
        printf("[PR][CCV] m_center_vol matrix:\n");
        MatView(m_center_vol,PETSC_VIEWER_STDOUT_WORLD);
    #endif
 ...
 * @output:
 *          two vector for this target group
 *          (I skip calculating the IS directly, since I need the vector
 *          for finding the real index of SV_indices)
 *
 */
void Partitioning::create_group_index(int target_group,
                           std::vector<std::pair<int, int> > v_groups,
    ...
    //it might get bigger but I try to use limited knowledge here
    v_p_index.reserve(ncols);
    for(j=0; j< ncols; j++ ){
        v_p_index.push_back(cols[j]);
    }
    MatRestoreRow(m_parts_p, v_groups[target_group].first, &ncols, &cols,
                  &vals);
    ...
    MatRestoreRow(m_parts_n, v_groups[target_group].second, &ncols, &cols,
                  &vals);

#if dbl_PR_CGI >= 5      // default 5
    // - - - - - - Print the final vectors - - - - - -
    cout<< "target is: "<< target_group <<endl;
    cout<< "v_p_index size: "<< v_p_index.size() <<endl;
    for(auto it = v_p_index.begin(); it != v_p_index.end(); it++ ){
        cout<< *it << ",";
    }
    cout<< endl;
    cout<< "v_n_index size: "<< v_n_index.size() <<endl;
...
void Partitioning::calc_performance_measure(Mat& m_TD,
                                std::vector<Mat>& v_mat_avg_center,
                                std::vector<Mat>& v_mat_predicted_labels,
                                summary& result_summary){
    // read the target labels from m_TD to a vector
    //  (test data matrix has the labels in the first column)
    ...
    std::vector<double> v_final_predicted_label(num_points,0);

/*
 * 1- majority voting all the same
 * 2- majority voting with inverse Euclidean distance weight (to avg_centers)
 * 3- majority voting with inverse Euclidean distance weight to the 2nd power
 * 4- majority voting with inverse Manhatan distance weight
 * 5-
 */
    int maj_id = paramsInst->get_pr_maj_voting_id();
    switch(maj_id){
        case 1:
            calc_majority_voting(v_mat_predicted_labels, num_points,
                                 v_final_predicted_label);
            break;
        case 2 ... 4:
            calc_majority_voting_distance_weight(v_mat_predicted_labels,
                                    num_points, m_TD, v_mat_avg_center,
                                    maj_id, v_final_predicted_label);
            break;
        case 5: // not implemented yet
            calc_majority_voting_nearest(v_mat_predicted_labels, num_points,
                                    m_TD, v_mat_avg_center, maj_id,
                                    v_final_predicted_label);
            break;
        default:
            std::cerr << "ERROR: wrong majority voting is selected!" << endl;
            exit(1);
    }
    cout << "[PR][CPM] v_final_predicted_label.size() "<<
            v_final_predicted_label.size() << " v_final_predicted_label[0]: "<<
            v_final_predicted_label[0] <<endl;
...
#if dbl_PR_CPM >= 5
        cout << "[PR][CPM] labels =>  target: "<< v_target_labels[i] <<
                " predicted: " << v_final_predicted_label[i] <<endl;
#endif
...
void Partitioning::calc_majority_voting(std::vector<Mat>& v_mat_predicted_labels,
                                int num_TD_points,
    ...
    for(unsigned int i =0; i < v_mat_predicted_labels.size(); i++){
        ...
        //calc sum of each row of predict_label transpose
        MatGetRowSum(m_pl_t, v_sum_labels);
        MatDestroy(&m_pl_t);

        PetscScalar     *arr_predicted_labels;
        PetscMalloc1(num_TD_points, &arr_predicted_labels);

        //the vector od sum_labels is calculated separately for each iteration
...
void Partitioning::calc_majority_voting_distance_weight(
                                std::vector<Mat>& v_mat_predicted_labels,
                                int num_TD, Mat& m_TD,
                                std::vector<Mat>& v_mat_AC,
                                int distance_id,
                                std::vector<double>& v_final_predicted_label ){
    ...
    // the default is also initialize it with zeros
    std::vector<double> v_sum_norm_prediction(num_TD,0.0);
    // loop over multiple prediction matrix and avg_center
    for(unsigned int iter =0; iter < num_iter; iter++){
        MatGetSize(v_mat_AC[iter], &num_AC, NULL);
//        cout << "[PR][CMVDW] [DEBUG] m_AC_num_row:" << num_AC <<
//                " num_TD_row:" << num_TD  <<
//                " v_mat_AC.size():" << v_mat_AC.size() << endl;
        Mat m_PLt;
        ...
        //vector of temporary inverse distances from a test point to all avg_centers
        std::vector<double> v_tmp_inv_dist(num_AC);

        //Calculate the inverse of the distance between the test point and all
        // the avg_centers into v_tmp_inv_dist
        for(i=0; i< num_TD; i++){
            //Test Data points
            MatGetRow(m_TD,i,&ncols_TD, &cols_TD, &vals_TD);

            //Predicted Labels
            MatGetRow(m_PLt,i,&ncols_PLt, &cols_PLt, &vals_PLt);
            for(j=0; j< num_AC; j++){

                //Average Centers
                MatGetRow(v_mat_AC[iter],j,&ncols_AC, &cols_AC, &vals_AC);

                //case 1 is handeled in another function
                //  (calc_majority_voting function)
                switch(distance_id){
                    case 1:
                        std::cerr <<"ERROR: [PR][CMVDW] wrong function is called"<<
                                   endl;
                        exit(1);
                        break;
                    case 2:       //Euclidean distance
                        //get distance
                        tmp_dist = cf.calc_euclidean_dist(ncols_TD, ncols_AC,
                                            cols_TD, cols_AC, vals_TD, vals_AC);
                        //save inversed distance
                        v_tmp_inv_dist[j]= 1.0 / tmp_dist;
                        break;
                    case 3:       //(Euclidean distance)^2
                        tmp_dist = cf.calc_euclidean_dist(ncols_TD, ncols_AC,
                                            cols_TD, cols_AC, vals_TD, vals_AC);
                        //save inversed distance
                        v_tmp_inv_dist[j]= 1.0 / pow(tmp_dist, 2);
                        break;
                    case 4:       // Manhatan distance (Norm 1)
                        tmp_dist = cf.calc_manhatan_dist(ncols_TD, ncols_AC,
                                            cols_TD, cols_AC, vals_TD, vals_AC);
                        //save inversed distance
                        v_tmp_inv_dist[j]= 1.0 / tmp_dist;
                        break;
                    default:
                        std::cerr <<"ERROR:[PR][CMVDW] "<<
                                   "wrong function is called"<< endl;
                        exit(1);
                        break;
                }
                MatRestoreRow(v_mat_AC[iter],j,&ncols_AC, &cols_AC, &vals_AC);
            }
            // calc dot product of v_tmp_inv_dist and vector of
            //  predicted labels from PLt matrix
            tmp_final_label = cf.vec_vec_dot_product(ncols_PLt, cols_PLt,
                                                     vals_PLt, v_tmp_inv_dist);

            //normalize the weight vector(v_tmp_inv_dist)
            double sum_v_weights=0;
            std::for_each(v_tmp_inv_dist.begin(), v_tmp_inv_dist.end(),
                            [&] (double n) {
                sum_v_weights += n;
            });
            // v_sum_norm_prediction is initialized with zero in the beginning
            //  and sum up all the normalized predictions
    ...
    for(i=0; i< num_TD; i++){
        ...
        if( (v_sum_norm_prediction[i] / (double)num_iter) > 0)
...
//        cout << "[PR][CMVDW] final_predicted_label:" <<
//                v_final_predicted_label[i] << endl;
...
/* 051917-2350
 * @input:
 * ُThe distance_id has not used for this method.
 * Default is Euclidean distance
 * (it could be used in future to try different distances )
 *
 * std::vector<Mat>& v_mat_predicted_labels this should be modified to 1 matrix
 *      rather than vector of matrices, but since the refinement has not
 *      modified yet, I keep it like this for now
 * the predicted labels have been calculated by all the models and now we just
 *      need to select/filter/aggregate them to one label
 * @output: vector of labels is the output (v_final_predicted_label)
 */
void Partitioning::calc_majority_voting_nearest(
            std::vector<Mat>& v_mat_predicted_labels, int num_TD, Mat& m_TD,
    ...
    for(unsigned int iter =0; iter < num_iter; iter++){
        MatGetSize(v_mat_AC[iter], &num_AC, NULL);
        cout << "[PR][CMVN] [DEBUG] m_AC_num_row:" << num_AC <<
                " num_TD_row:" << num_TD  << " v_mat_AC.size():" <<
                v_mat_AC.size() << endl;
        Mat m_PLt;
        //Transpose the PL, to get predicted labels for TD points as row
        MatTranspose(v_mat_predicted_labels[iter],MAT_INITIAL_MATRIX,&m_PLt);
        ...
        cf.exp_matrix(m_PLt, "./data/", m_fname, "CMVN");
        ...
        for(i=0; i< num_TD; i++){
            ...
            //set the min_distance to +infinity for each data point
            min_distance= std::numeric_limits<double>::max();
            for(j=0; j< num_AC; j++){           //number of models
                //Average Centers
                MatGetRow(v_mat_AC[iter],j,&ncols_AC, &cols_AC, &vals_AC);
                //calculate the distance of current test point to avg_center
                //  of current level
                tmp_dist = cf.calc_euclidean_dist(ncols_TD, ncols_AC, cols_TD,
                                                  cols_AC, vals_TD, vals_AC);
                cout << "[PR][CMVN] [DEBUG] i:"<<i<<", j:"<<j<<", tmp_dist:" <<
                        tmp_dist << ", min_dist:" << min_distance;
                cout << ", v_final_predicted_label[i]:" <<
                        v_final_predicted_label[i] << endl;
                if(tmp_dist <= min_distance){
                    min_distance = tmp_dist;           // update the minimum
                    //make sure the i is the test point id which is predicted
                    // by current model (we are in the right row for sure)

                    // update the final label
                    v_final_predicted_label[i] = vals_PLt[i];
                    cout << "[PR][CMVN] [DEBUG] update the label with closer"<<
                            " model, new lable" << v_final_predicted_label[i]<<
                            ", vals_PLt[i]:" << vals_PLt[i] << ",ncols_PLt:" <<
                            ncols_PLt <<", cols_PLt[i]:" << cols_PLt[i] <<endl;
                }
                MatRestoreRow(v_mat_AC[iter],j,&ncols_AC, &cols_AC, &vals_AC);
            }
            cout << "[PR][CMVN] [DEBUG] final prediction -> "<<
                    "v_final_predicted_label[i]:" <<
                    v_final_predicted_label[i] << endl;
            MatRestoreRow(m_PLt,i,&ncols_PLt, &cols_PLt, &vals_PLt);
            MatRestoreRow(m_TD,i,&ncols_TD, &cols_TD, &vals_TD);
        } // end of prediction for all the test data points
        MatDestroy(&m_PLt);
        exit(1);
        cout << "[PR][CMVN] iter:" << iter << endl;      //$$debug
    }// end of iterations

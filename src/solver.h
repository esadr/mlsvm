...
#define weight_instance 1
    ...
    // LIBSVM with instance weights support at libsvm-3.20
    #include "svm_weighted.h"
#else
...
private:
    ...
    void PD_read_problem_index_base(Mat& m_data,
                                    std::vector<int>& v_lbl,
                                    const PetscScalar * arr_index,
                                    int num_nnz);

    void read_problem(Mat& m_train_data_p, Vec& v_vol_p,
                      Mat& m_train_data_n, Vec& v_vol_n);

    void read_problem_without_instance_weight(Mat& m_train_data_p,
                                              Mat& m_train_data_n);

    void set_weights_num_points(svm_parameter& param_,
                                PetscInt num_p_point,
                                PetscInt num_n_point);

    void set_weights_sum_volume(svm_parameter& param_,
                                Vec& v_vol_p, Vec& v_vol_n);

    void set_weights_sum_volume_index_base(svm_parameter& param_,
                                Vec& v_vol_p, Vec& v_vol_n,
                                std::vector<PetscInt>& v_p_index,
                                std::vector<PetscInt>& v_n_index,
                                PetscInt iter_p_end, PetscInt iter_n_end);

    void PD_set_weights_sum_num_point_IB(svm_parameter& param_,
                                         std::vector<int>& v_lbl,
                                         const PetscScalar * arr_index,
                                         int num_nnz); //personalized classifier
...
public:
    Solver(){
        test_dataset_f_name = Config_params::getInstance()->
                get_test_ds_f_name().c_str();

        predict_probability = Config_params::getInstance()->
                get_svm_probability();
    }
    ...
    svm_model * train_model(Mat& m_data_p, Vec& v_vol_p,
                            Mat& m_data_n, Vec& v_vol_n,
                            bool inherit_params, double param_c,
                            double param_gamma);

    svm_model * train_model_index_base(Mat& m_data_p, Vec& v_vol_p,
                                       Mat& m_data_n, Vec& v_vol_n,
                                       std::vector<PetscInt>& v_p_index,
                                       std::vector<PetscInt>& v_n_index,
                                       PetscInt iter_p_end, PetscInt iter_n_end,
    ...
    void stand_alone_train_without_instance_weight(Mat& m_data_p,
                                                   Mat& m_data_n,
                                                   std::string model_fname);

    void stand_alone_train_instance_weight(Mat& m_data_p , Vec& v_vol_p,
                                           Mat& m_data_n, Vec& v_vol_n,
                                           std::string model_fname);
    ...
    void partial_solver(Mat& p_data, Vec& v_vol_p, Mat& n_data,
    ...
    void combine_test_data(Mat& test_total, Mat& dt_test_p,
                           PetscInt size_p, Mat& dt_test_n,
                           PetscInt size_n, PetscInt max_num_col_);


    void predict_test_data_in_matrix_output(Mat& test_data,
                                            int target_row,
                                            Mat& m_predicted_label);

    void predict_VD_in_output_matrix(Mat& m_VD_p,Mat& m_VD_n,
                                     int target_row,
                                     Mat& m_predicted_label);
...
//    std::map<measures,double> evaluate_testdata(int level);
    ...
    void evaluate_testdata(Mat& untouched_test_data, int level,
                           summary& final_summary);

    void test_predict(Mat& test_data, summary& result_summary,
                      int iteration=-1, bool export_prediction=0,
                      bool exp_decision_val=0);
    ...
    void test_predict_index_base(Mat& m_data_p, Mat& m_data_n,
                                 std::vector<PetscInt>& v_p_index,
                                 std::vector<PetscInt>& v_n_index,
                                 PetscInt iter_p_end, PetscInt iter_n_end,
                                 summary& result_summary, int iteration=-1);
    ...
    //personalized classifier
    void PD_test_predict_index_base(Mat& m_data, std::vector<PetscInt> v_lbl,
                                const PetscScalar * arr_train_index,
                                PetscInt idx_start_test, PetscInt idx_end_test,
                                summary& result_summary, int iteration);
    ...
    void prepare_solution_single_model(svm_model * model_,
                                       int num_point_p,
    ...
    void get_separate_matrices(Mat& m_data, std::vector<int>& v_target_lbl,
                               const PetscScalar * arr_train_index,
                               PetscInt num_nnz, Mat& m_p_data, Mat& m_n_data);

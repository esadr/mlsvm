...
//    exit(1);
    ...
    m_VD_p = cf.sample_data(m_min_full_data,
                paramsInst->get_ms_VD_sample_size_fraction(),
...
//    exit(1);
    ...
    t_sample.stop_timer("[MC] validation data is sampled from the finest "
                        "training data");
//    exit(1);

    //====================== check test data file =============================
    assert(paramsInst->get_test_ds_f_name() != "" &&
...
//    exit(1);
    ...
    #endif
    t_iteration.stop_timer("[MC] Vcycle (including loading datasets) "
                           "at iteration ", std::to_string(i));
    //save the final metadata for models of the current k-fold
    ...
    //in case of exporting the data, the models are not trained and
    //   the output is useless
#endif //export_SVM_models
#endif //dbl_exp_train_data

    printf("[MC] MLSVM separate test data classifier finished successfully!\n");

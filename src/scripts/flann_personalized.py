...
Version 1
    The script reads 3 parameters from command line which are path and filename and number of neighbors
...
def run_flann(file_path, file_name, num_NN, dist_type,output_file_with_path):
    ...
    print("num_row is %d, num_NN is %d"% (num_row, int(num_NN)));
    ...
    try:
        ...
        print("\n\nException in running Flann, error: ")
        print(sys.exc_info())
        exit(1)
...
#    m_dists_sparse=  scipy.sparse.csr.csr_matrix(m_dists);
    stop = timeit.default_timer()
    print("Convert the ndarray to sparse matrix takes: %.1f seconds" % (stop - start))


    # save the result
    try:
        no_suffix_name = file_name.split(".")[0]
#        print('no suffix name :%s' % no_suffix_name)
        start = timeit.default_timer()
        #result_outputfile = file_path+''+ no_suffix_name+'_indices.dat'
#        result_outputfile = file_path+''+ file_name+'_indices.dat'
#        result_outputfile = './data/'+ no_suffix_name +'_indices.dat'
        print('Outputing Data: '+output_file_with_path)
        result_file = open(output_file_with_path,'w')
...
if __name__ == '__main__':
    ...
    if debug:       # ----------------------- get parameters for debug -------------------------------
        ...
        if (len(argv)) < 5:
            print ("please enter path and file_name and number of NN")
            print("Example: /data/twonorm_min_data.dat int_num_NN int_dist_type /out_path/out_filename \nExit due to lack of parameters!")
            exit(1)
        else:
            try:
                ...
                num_NN = argv[2]
                dist_type = argv[3]
                output_file_with_path = argv[4]
            except:
                print("\n\nException on input path, file name or number of NN and distance type, try again!")
                exit(1)

    # --------------------- After reading parameters, run the script  ------------------------
    print("Input filename is: "+file_path+ file_name)
    run_flann(file_path, file_name, num_NN,dist_type,output_file_with_path)

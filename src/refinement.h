...
public:
    ...
    solution main(Mat& m_data_p, Mat& m_P_p, Vec& v_vol_p,
                  Mat&m_WA_p,Mat& m_data_n, Mat& m_P_n,
                  Vec& v_vol_n, Mat&m_WA_n,Mat& m_VD_p, Mat& m_VD_n,
                  solution& sol_coarser,int level,
                  std::vector<ref_results>& v_ref_results);

    void find_SV_neighbors(Mat& m_data, Mat& m_P,
                  std::vector<int>& seeds_ind, Mat& m_WA,
                  Mat& m_neighbors, std::string cc_name,
                  IS& IS_neigh_id);

    void process_coarsest_level(Mat& p_data, Vec& v_vol_p,
                                Mat& n_data, Vec& v_vol_n,
                                Mat& m_VD_p, Mat& m_VD_n, int level,
                                solution& sol_coarsest,
                                std::vector<ref_results>& v_ref_results);

    void prepare_single_solution(svm_model **svm_trained_model,
                                 int num_row_p,

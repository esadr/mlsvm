...
//    save_model(model_fname.c_str(), local_model);
...
void SolverLinear::stand_alone_train_linear(Mat& m_data_p , Vec& v_vol_p, Mat& m_data_n, Vec& v_vol_n, std::string model_fname){
    ...
    std::cout << "[SV][SATL] param.solver_type:" << param.solver_type << std::endl;
    std::cout << "[SV][SATL] param.C:" << param.C << std::endl;
    std::cout << "[SV][SATL] param.eps:" << param.eps << std::endl;
    const char *error_msg;                                  //check parameters
...
//    local_model = svm_train(&prob,&param);
//    save_model("./debug/single_level.model",local_model);
...
////             exit(1);
//            //            save_model("./final_model.svm",model);
//        }
